...
Purpose: authenticated platform users.

Fields:

- `id`: UUID, primary key
- `email`: text, unique, nullable only when phone-only auth is explicitly supported
- `phone`: text, nullable
- `first_name`: text, nullable
- `last_name`: text, nullable
- `status`: enum `UserStatus`
- `password_hash`: text, nullable for external-auth users
...
- organization admins manage memberships, not global user identity
- platform admins may suspend users
